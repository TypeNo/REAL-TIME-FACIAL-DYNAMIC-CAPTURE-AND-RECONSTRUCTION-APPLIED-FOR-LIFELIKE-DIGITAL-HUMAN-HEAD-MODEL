...
//static Model model;
Model* model = nullptr;  // pointer, can be null initially
...
int main()
{
    ...
    try {
        ...
        //model = std::move(Model("Animation.glb"));
        model = new Model("Animation.glb");
...
void DrawModelView() {
    ...
    //static Shader shader("shaders/vertex.glsl", "shaders/fragment.glsl");
    //static Model model1("Animation.glb");
    ...
    // Bind the morph weights SSBO to binding point 0 (matches shader)
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 0, model->GetMorphSSBO());
